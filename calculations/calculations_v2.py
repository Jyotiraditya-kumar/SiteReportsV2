...
def cool_decorator(func):
    def wrapper(*args, **kwargs):
        ...
        for geometry_col in geometry_cols:
            out_dir = OD / geometry_col
            out_dir.mkdir(parents=True, exist_ok=True)
            poly = stores[geometry_col].values.tolist()[0]
            poly = shapely.from_wkt(poly)
            res[geometry_col] = func(poly)
        return {'data':res}

    return wrapper


def get_distance_from_site(df):
    stores = read_catchment_file().dropna(subset=['lat'])
...
def get_projects(poly):
    ...
    joined_ = stores_gdf.sjoin(projects_gdf, how="inner", predicate="contains").sort_values(by="final_num_units",
                                                                                            ascending=False)
    joined_ = get_distance_from_site(joined_)
...
def get_competition(poly):
    ...
    joined = get_distance_from_site(joined)
...
@cool_decorator
def get_rent_and_sale_prices(poly):
...
@cool_decorator
def get_high_streets(poly):
    df = pd.read_csv("data/blr_cluster_geom.csv")[['cluster_name', 'locality', 'polygon_updated', 'area', 'lat', 'lng']]
